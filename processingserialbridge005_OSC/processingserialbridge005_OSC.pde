...
import oscP5.*; //import OSC libraries
import netP5.*;
...
void setup() {
  ...
  proc1 = new NetAddress("127.0.0.1", 12322);
  
  osc.plug(this, "getosc", "/getosc");
...
void getosc(String hdr) {

  int hdrnbr = 0;
  for (int i=0; i<hdrs.length; i++) {
    if ( hdrs[i].equals(hdr) ) {
      hdrnbr = i;
      break;
    }
  }

  //Send values & headers via OSC
   osc.send("/"+hdr, new Object[]{datanorm[hdrnbr]}, sc);
} //end getosc
